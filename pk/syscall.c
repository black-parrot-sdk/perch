...
int sys_renameat(int old_fd, const char *old_path, int new_fd, const char *new_path) {
  ...
  if(old_kfd != -1 && new_kfd != -1) {
    ...
    if (!strncpy(kold_path, old_path, MAX_BUF) || !strncpy(knew_path, new_path, MAX_BUF))
      return -ENAMETOOLONG;
...
long sys_linkat(int old_dirfd, const char* old_name, int new_dirfd, const char* new_name, int flags)
{
  ...
  if (old_kfd != -1 && new_kfd != -1) {
    ...
    if (!strncpy(kold_name, old_name, MAX_BUF) || !strncpy(knew_name, new_name, MAX_BUF))
      return -ENAMETOOLONG;
...
int sys_rt_sigaction(int sig, const void* act, void* oact, size_t sssz)
{
  if (oact) {
    long koact[3] = {0};
    memcpy(oact, koact, sizeof(koact));
  }
...
ssize_t sys_writev(int fd, const long* iov, int cnt)
{
  ...
  for (int i = 0; i < cnt; i++) {
    long kiov[2];
    memcpy(kiov, iov + 2*i, 2*sizeof(long));
...
void bp_handle_syscall(uint64_t *regs, uint64_t mcause, uint64_t mtval) {
